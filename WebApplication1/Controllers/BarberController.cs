...
namespace WebApplication1.Controllers
{
	public class BarberController : Controller
	{
		...
		public JsonResult GetEstimatedTime()
		{
			...
			return Json(string.Join("   |   ", barbersAndWaitTimes.Select(s => s.Item1 + " has wait of " + s.Item2 + " minutes.")), "text/html");
		}
			...
			var barbersAndWaitTimes = c_listOfBarbers.Select(barber => new
			{
				...
				WaitTimes = scheduledCustomers
					.Where(w =>
						...
						(w.BarberNameRequested == barber && (w.CurrentBarberAssigned is null || w.CurrentBarberAssigned == barber) ) ||
		...
		private List<ScheduledCustomerModel> scheduledCustomers
		{
			get {
				...
				List<ScheduledCustomerModel> customers = Session["ScheduledCustomers"] as List<ScheduledCustomerModel>;
			...
			set {
				...
				foreach (ScheduledCustomerModel customer in customers)
				{
					...
					if (customer.CurrentBarberAssigned == null)
					{
						decimal countCustomersAhead = scheduledCustomers
							.Where(w => (w.AppointmentID < customer.AppointmentID && w.AppointmentID != customer.AppointmentID) // all customers with lower AppointmentID that aren't the same customer.
									|| w.CurrentBarberAssigned == customer.BarberNameRequested) // possible that a customer at end of queue is bumped up into barber chair.
				...
				Session["ScheduledCustomers"] = customers.OrderBy(o => o.WaitTime).ThenBy(tb => tb.BarberNameRequested).ToList();
			}
		}
